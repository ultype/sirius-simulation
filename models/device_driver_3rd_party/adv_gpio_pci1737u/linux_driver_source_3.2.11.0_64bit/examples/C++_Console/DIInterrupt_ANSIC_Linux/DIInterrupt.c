...
//-----------------------------------------------------------------------------------
#define  deviceDescription  L"DemoDevice,BID#0"	// You have to change this to the correspnded one. # CAT /sys/class/daq/daq0/desc
int32    startPort = 0;
int32    portCount = 1;
...
int main(int argc, char* argv[])
{
	...
	ICollection * interruptChans = NULL;
	PortDirection * portDir = NULL;
	ICollection * ports = NULL;

	// Step 1: Create a 'InstantDiCtrl' for DI function.
	InstantDiCtrl * instantDiCtrl = AdxInstantDiCtrlCreate();
	
	// Step 2: Set the notification event Handler by which we can known the state of operation effectively.
	InstantDiCtrl_addInterruptListener(instantDiCtrl, OnDiSnapEvent);
	...
	{	
		...
		devInfo.ModuleIndex = 0;	//Keep this always be 0
		...
		diIntChannel = (DiintChannel *)ICollection_getItem(interruptChans, 0);
		...
		// set direction of port1 as Output
		ports = InstantDiCtrl_getPortDirection(instantDiCtrl);
		portDir = (PortDirection *)ICollection_getItem(ports, 1);
		ret = PortDirection_setDirection(portDir, DioPortDir.Output);
		CHK_RESULT(ret);
