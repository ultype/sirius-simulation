...
struct pci_device_id daq_device_ids[] = {
   { PCI_DEVICE(ADVANTECH_VID, DEVICE_ID) },
   { 0, }
};
...
#define CHK_RESULT(_ok_, err, err_label)   if (!(_ok_)) { ret = err; goto err_label; }
   ...
   /* request I/O regions */
   CHK_RESULT(\
      request_region(shared->IoBase, shared->IoLength, DEVICE_NAME),\
   ...
   tasklet_init(&daq_dev->dio_tasklet, daq_dio_tasklet_func, (unsigned long)daq_dev);
