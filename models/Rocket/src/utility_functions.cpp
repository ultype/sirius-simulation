               ...
               const double &arg_peri)
{
    ...
    TIP.assign_loc(0, 0,
                   clon_anode * carg_peri - slon_anode * sarg_peri * cincl);
    TIP.assign_loc(0, 1,
                   -clon_anode * sarg_peri - slon_anode * carg_peri * cincl);
    TIP.assign_loc(0, 2, slon_anode * sincl);
    TIP.assign_loc(1, 0,
                   slon_anode * carg_peri + clon_anode * sarg_peri * cincl);
    TIP.assign_loc(1, 1,
                   -slon_anode * sarg_peri + clon_anode * carg_peri * cincl);
...
//  up to 85 km.
// *Extrapolation above 71 km and beyond 85 km is carried out from 71 km
//  altitude
...
//  one for altitudes less than 86 kilometers and the other for higher
//  altitudes.
...
int us76_nasa2002(double z,  /* altitude (km)        */
                  ...
                  )
{
    ...
    double rs = 8314.32;  /* universal gas constant (m^2/s^2) (gram/mole) /
                             (degree Kelvin) */
    ...
    if (i < 7) {
        ...
        if (g < 0. || g > 0.) {
            *p = ps[i] * pow((tms[i] / (tms[i] + g * (ht - zl))),
                             ((go * wmo) / (rs * g * 0.001))) *
                 100.;
        } else {
            *p = ps[i] *
...
//Read table & store table's data
void read_tables(char *file_name,Datadeck &datatable)
{
