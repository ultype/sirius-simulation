...
void GPS_constellation::readfile(const char *fname) {
    ...
    if (neph <= 0) {
        std::cout << "Error : Can not find the RINEX Navigation file !!" << std::endl;
        exit(1);
    }
}
...
void GPS_constellation::compute() {
    ...
    nsat = allocateChannel(chan, eph[ieph], ionoutc, time->get_gpstime(), SBEE, 0.0);
...
void GPS_constellation::show() {
    // std::cout << "Lock number =" << nsat << std::setw(2) << std::left << '\t' << "GPS SOW =" << time->get_gpstime().get_SOW() << std::setw(2) << std::left << '\t' << "GDOP = " << gdop << std::endl;
    // std::cout << "/******************************************************************************************************************/" << std::endl;
    // std::cout << "Pseudo-Number" << std::setw(2) << std::left << '\t' << "Elevation" << std::setw(2) << std::left << '\t' << "Azimuth" << std::setw(2) << std::left << '\t' << "Distance"
    // << std::setw(2) << std::left << '\t' << "Ion Delay" << std::setw(2) << std::left << '\t' << "Pseudo-Range" << std::setw(2) << std::left << '\t' << "Pseudo-Range rate" << std::endl;
    // for (int i = 0; i < MAX_CHAN; i++) {
    //     if (chan[i].prn > 0) {
    //         std::cout << chan[i].prn << std::setw(10) << std::left << '\t' << (chan[i].azel(1)) * DEG << std::setw(5) << std::left << '\t'
    //         << (chan[i].azel(0))*DEG << std::setw(5) << std::left << '\t' << chan[i].rho0.d << std::setw(8) << std::left << '\t'
    //         << chan[i].rho0.iono_delay << std::setw(5) << std::left << '\t' << chan[i].rho0.range << std::setw(5) << std::left << '\t' << chan[i].rho0.rate << std::endl;
    //         }
    // }
    // std::cout << "/******************************************************************************************************************/" << std::endl;
    fprintf(stderr, "*");
...
int GPS_constellation::allocateChannel(channel_t *chan, ephem_t *eph, ionoutc_t ionoutc, time_util::GPS_TIME grx, arma::vec3 XYZ, double elvMask) {
    nsat = 0;
    int i, sv;
    arma::vec2 azel;
...
int GPS_constellation::checkSatVisibility(ephem_t eph, time_util::GPS_TIME g, arma::vec3 xyz, double elvMask, arma::vec2 &azel) {
    arma::vec3 llh;
    arma::vec3 ned;
    arma::vec3 pos;
    arma::vec3 vel;
    ...
    arma::mat33 BODYENU = enu2atenna(phi * RAD, tht * RAD, psi * RAD);  // Body to ENU
    arma::mat33 ANTBODY = enu2atenna(0.0 * RAD, -20.0 * RAD, 0.0 * RAD);  // Antenna to Body
 ...
 */
void GPS_constellation::computeRange(range_t *rho, ephem_t eph, ionoutc_t *ionoutc, time_util::GPS_TIME g, arma::vec3 xyz) {
    arma::vec3 pos;
    arma::vec3 vel;
    arma::vec2 clk;
    arma::vec3 los;
...
void GPS_constellation::satpos(ephem_t eph, time_util::GPS_TIME g, arma::vec3 &pos, arma::vec3 &vel, arma::vec2 &clk) {
...
double GPS_constellation::ionosphericDelay(const ionoutc_t *ionoutc, time_util::GPS_TIME g, arma::vec3 llh, arma::vec2 azel) {
...
double GPS_constellation::subGpsTime(time_util::GPS_TIME g1, time_util::GPS_TIME g0) {
    double dt;

    dt = g1.get_SOW() - g0.get_SOW();
    dt += static_cast<double>(g1.get_week() - g0.get_week()) * time_util::SEC_PER_WEEK;
...
//  g0.get_SOW() = (double)(((unsigned long)(g.get_SOW()+0.5))/30UL) * 30.0; // Align with the full frame length = 30 sec
