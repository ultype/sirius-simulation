...
void GPS_Receiver::update_markov(double int_step) {
    ...
    ucfreq_noise = markov(ucfreq_noise_sigma, ucfreq_noise_bcor, get_elapsed_time(), int_step, ucfreq_noise);
    for (i = 0; i < 4; i++) {
        PR_NOISE[i] = markov(PR_NOISE_sigma[i], PR_NOISE_bcor[i], get_elapsed_time(), int_step, PR_NOISE[i]);
    }
    for (i = 0; i < 4; i++) {
        DR_NOISE[i] = markov(DR_NOISE_sigma[i], DR_NOISE_bcor[i], get_elapsed_time(), int_step, DR_NOISE[i]);
    }
}
...
void GPS_Receiver::get_quadriga() {
    ...
    if (visible_count < 4) {  // re-acquiring GPS if not enough SVs visible (less than 4)
        gps_epoch = get_elapsed_time();
        gps_acq = false;
