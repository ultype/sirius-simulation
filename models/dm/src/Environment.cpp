...
void Environment::set_wind_turbulunce(double turb_length, double turb_sigma,
    ...
    if (wind)
        wind->enable_turbulance(turb_length, turb_sigma, taux1, taux1d, taux2, taux2d, tau, gauss_value);
}
...
void Environment::propagate(double int_step) {
    ...
    // arma::vec3 VBAB = kinematics->get_VBAB();
...
void Environment::dm_RNP() {
    ...
    ***/
    ...
    index = static_cast<int32_t>(time->get_modified_julian_date().get_mjd() - 55197.00);   /* get dUT1 = Ut1 - UT from table*/
