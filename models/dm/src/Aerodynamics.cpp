...
AeroDynamics::AeroDynamics(Propulsion &prop)
    :   propulsion(&prop),
    VECTOR_INIT(xcp, 3) {
}
...
void AeroDynamics::calculate_aero(double int_step) {
    ...
    if (liftoff == 1) {
        ...
        double betax  = grab_betax();

        double rho    = grab_rho();
        double vmach  = grab_vmach();
        double pdynmc = grab_pdynmc();
        double tempk  = grab_tempk();
        ...
        // cll = 0.0;
        // clm = cmq * ((WBIB(1) * refd) / (2.0 * dvba));
        // cln = cmq * ((WBIB(2) * refd) / (2.0 * dvba));

        cll = 0.0;
        clm = cn * ((-xcg(0) / refd) - xcp(0)) * cphip + cmq * ((WBIB(1) * refd) / (2.0 * dvba));
        cln = -cn * ((-xcg(0) / refd) - xcp(0)) * sphip + cmq * ((WBIB(2) * refd) / (2.0 * dvba));
        // cll = 0.0;
        // clm = 0.0;
        // cln = 0.0;
        // double cn = 0;
